#!/usr/bin/python                                                                                                                                 
# Simple script to make sure that run is going and the fesimdaq program is going...                                                               

import urllib2
import json
import time
from subprocess import Popen, PIPE
from time import localtime, gmtime, strftime





# Get Program/fesimsaq                                                                                                                            
simdaqODB = urllib2.urlopen("http://localhost:8082/?cmd=jcopy&encoding=json&odb=/Equipment/BeamlineEpics/Settings").read()


json_obj = json.loads(simdaqODB)['Names']

init_str = ""
branch_str = ""
assign_str = ""

index = -1;
for oname in json_obj:
    index = index + 1
    if "Default" in oname or "default" in oname:
        continue
    name = oname.replace(":","_").replace(".","_")
    # print name,index
    init_str += "  double " + name + ";\n"
    branch_str += '  tBeamline->Branch("'+name + '",&' + name + ', "' + name + '/D");\n'
    assign_str += '  ' + name + ' =  data->GetFloat()[' + str(index) + '];\n'


print "// midas2root code fragments generated by epics2mdias/generate_epics_code_fragment.py"
print init_str
print branch_str
print assign_str

                         



