...
const char *frontend_name = "feUCN2Pha5Ot2Ep";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2Pha5Last", epics_ca, 102, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2EpPha5Last",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpPha5Last/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpPha5Last/variables/measured", &hRespond);
