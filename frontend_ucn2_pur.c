...
const char *frontend_name = "feUCN2Pur";
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2Pur/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2Pur/variables/measured", &hRespond);
