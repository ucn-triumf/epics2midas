...
const char *frontend_name = "feUCN2OtherEpics";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2Other", epics_ca, 102, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2EpicsOther",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpicsOther/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpicsOther/variables/measured", &hRespond);
