...
const char *frontend_name = "feUCN2Pha5TmpEp";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2Pha5Tmp", epics_ca, 102, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2EpPha5Tmp",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpPha5Tmp/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpPha5Tmp/variables/measured", &hRespond);
