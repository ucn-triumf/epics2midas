...
const char *frontend_name = "feUCN2fastEpics";
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
	...
	size = sizeof(float);
	db_get_data_index(hDB, hRespond, &cat, &size, 9, TID_FLOAT);
	//if (abs(cat - dog) > 10.f)
	//cm_msg(MINFO,"feEpics","R/W Access to Epics is in jeopardy!");
	
	db_set_data_index(hDB, hWatch, &dog, sizeof(float), 9, TID_FLOAT);
      }
