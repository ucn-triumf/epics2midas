...
const char *frontend_name = "feUCN2Phase2BEpics";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2Phase2B", epics_ca, 102, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2EpicsPhase2B",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpicsPhase2B/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpicsPhase2B/variables/measured", &hRespond);
