...
const char *frontend_name = "feUCN2FastEpic";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2FastEpic", epics_ca, 11, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2FastEpic",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2FastEpic/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2FastEpic/variables/measured", &hRespond);
	...
	size = sizeof(float);
	db_get_data_index(hDB, hRespond, &cat, &size, 10, TID_FLOAT);
	//if (abs(cat - dog) > 10.f)
	//cm_msg(MINFO,"feEpics","R/W Access to Epics is in jeopardy!");
	
	db_set_data_index(hDB, hWatch, &dog, sizeof(float), 10, TID_FLOAT);
      }
