...
const char *frontend_name = "feUCN2LD2";
...
DEVICE_DRIVER epics_driver[] = {
  {"UCN2EpLD2", epics_ca, 65, NULL},  /* disable CMD_SET_LABEL */
  {""}
};

EQUIPMENT equipment[] = {

   {"UCN2EpLD2",                 /* equipment name */
...
INT frontend_loop()
{
  ...
  if (ss_time() - watchdog_time > 1)
      ...
      if (!hWatch)
	{
	  ...
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpLD2/variables/demand", &hWatch);
	  status = db_find_key(hDB, 0, "/equipment/UCN2EpLD2/variables/measured", &hRespond);
	...
	size = sizeof(float);
	db_get_data_index(hDB, hRespond, &cat, &size, 64, TID_FLOAT);
	//if (abs(cat - dog) > 10.f)
	//cm_msg(MINFO,"feEpics","R/W Access to Epics is in jeopardy!");
	
	db_set_data_index(hDB, hWatch, &dog, sizeof(float), 64, TID_FLOAT);
      }
